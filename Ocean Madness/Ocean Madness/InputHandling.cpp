...
void handleAsyncKeyboard(GLFWwindow* pWindow, double deltaTime)
{
    ...
    float objectMovementSpeed = 0.3f;
    float lightMovementSpeed = 0.3f;
...
//    }//if ( areAllModsUp(window) )...
    ...
    if ( cGFLWKeyboardModifiers::isModifierDown(pWindow, true, false, false ) )
    {
        ...
        {
            g_vec_pMeshes[g_selectedObject]->orientationXYZ.y += 0.03;
        }

        if (glfwGetKey(pWindow, GLFW_KEY_T) == GLFW_PRESS)
        {
            g_vec_pMeshes[g_selectedObject]->scale += 0.03;
        }
